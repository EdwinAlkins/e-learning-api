...
class Video(BaseModel):
    ...
    chapter_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey("chapter.id"), index=True)
...
class Chapter(BaseModel):
    ...
    formation_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey("formation.id"), index=True)
...
class Formation(BaseModel):
    ...
    chapters: so.Mapped[list[Chapter]] = so.relationship(Chapter, back_populates="formation")
