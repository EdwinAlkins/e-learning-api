...
@click.option(
    "--video-id",
    "-v",
    help="ID de la vidéo à transcrire (obtenu via --list)"
)
@click.option(
    "--list",
    "-l",
...
@click.option(
    "--formation",
    "-f",
    help="Transcrire toutes les vidéos d'une formation spécifique"
)
@click.option(
    "--skip-existing",
    "-s",
...
def main(
    ...
    if process_all or formation:
        ...
        if format_with_timecodes:
            click.echo(f"⏱️  Timecodes: {'au niveau des mots' if word_timecodes else 'au niveau des segments'}")
        click.echo()
