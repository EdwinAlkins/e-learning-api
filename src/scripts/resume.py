    ...
    for formation in catalog.formations:
        ...
        for chapter in formation.chapters:
            ...
            for video in chapter.videos:
                ...
                click.echo(f"    🎬 [{video.id}] {video.title} ({duration_min}m {duration_sec}s)")
        click.echo()
...
@click.option(
    "--video-id",
    "-v",
    help="ID de la vidéo à résumer (obtenu via --list)"
)
@click.option(
    "--list",
    "-l",
...
@click.option(
    "--formation",
    "-form",
    help="Résumer toutes les vidéos d'une formation spécifique"
)
@click.option(
    "--skip-existing",
    "-s",
...
def main(
    ...
    if process_all or formation:
        ...
        for idx, (vid_id, vid_path, vid_info) in enumerate(videos, 1):
            ...
            click.echo(f"Formation: {vid_info['formation']} | Chapitre: {vid_info['chapter']}")
            ...
            if not transcription_service.transcription_exists(vid_path):
                ...
                click.echo(f"💡 Pour créer la transcription, exécutez:\n   {get_transcription_command(vid_id)}\n")
            ...
            if not force and summary_service.summary_exists(vid_path):
                if skip_existing:
                    ...
                    click.echo("💡 Utilisez --force pour régénérer ou --skip-existing pour ignorer.\n")
                    skip_count += 1
                    continue
            ...
            try:
                ...
                click.echo(f"✅ Résumé généré et sauvegardé ({len(summary)} caractères)")
                success_count += 1
    ...
    if not video_path:
        click.echo(f"\n❌ Erreur: Vidéo avec l'ID '{video_id}' introuvable dans le catalogue.", err=True)
    ...
    try:
        ...
        click.echo(f"❌ Erreur: La transcription n'existe pas: {transcription_service.get_transcription_file_path(video_path)}", err=True)
        click.echo("\n💡 Pour créer la transcription, exécutez la commande suivante:\n")
        click.echo(f"   {get_transcription_command(video_id)}\n")
        sys.exit(1)
    except Exception as e:
